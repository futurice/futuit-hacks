...
PATH=/home/gsyncadmin/bin:/usr/local/bin:/usr/bin:/bin:/home/gsyncadmin/bin/gam
LOGFILE=/home/gsyncadmin/gam-scripts/gam_groupscript_delta.log
EXCLUSION_FILE=/home/gsyncadmin/gam-scripts/gam_group_exclusion.txt
...
if (( $(wc -l < .processed.tmp) > 1 )); then
  ...
  cat .processed.tmp |gam csv - gam update group ~Email who_can_post_message anyone_can_post 2>> $LOGFILE > /dev/null

  if [ "${DISABLE_SPAM_MODERATION}" = "true" ]; then
    echo "Disabling moderation queues for group batch." >> $LOGFILE
    # shellcheck disable=SC2002
    cat .processed.tmp |gam csv - gam update group ~Email spam_moderation_level allow 2>> $LOGFILE > /dev/null
  fi
else
